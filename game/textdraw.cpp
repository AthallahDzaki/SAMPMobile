...
{
	/*if (!TextDrawTexture[m_TextDrawData.iTextureSlot] == 0)
		return;*/
	...
	{
		DrawTextureUV(TextDrawTexture[m_TextDrawData.iTextureSlot], &rect,
					  m_TextDrawData.dwLetterColor,
					  m_TextDrawData.dwStyle == 5 ? uv_reflected : uv_normal);
	}
}
...
void CTextDraw::LoadTexture()
{
	...
	if (strlen(m_szText) < 64 && strchr(m_szText, '\\') == nullptr
		&& strchr(m_szText, '/') == nullptr)
	{
...
void CTextDraw::SnapshotProcess()
{
	...
	if (IsPedModel(m_TextDrawData.wModelID))
	{
		...
		snapshot = pSnapShotHelper->CreatePedSnapShot(m_TextDrawData.wModelID,
	...
	{
		snapshot = pSnapShotHelper->CreateVehicleSnapShot(m_TextDrawData.wModelID,
														  m_TextDrawData.dwBackgroundColor,
														  &m_TextDrawData.vecRot,
														  m_TextDrawData.fZoom,
	...
	else
	{
		...
		snapshot = pSnapShotHelper->CreateObjectSnapShot(m_TextDrawData.wModelID,
														 m_TextDrawData.dwBackgroundColor,
														 &m_TextDrawData.vecRot,
														 m_TextDrawData.fZoom);
	}
