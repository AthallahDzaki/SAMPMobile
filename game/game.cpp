...
void CGame::RemoveBuilding(uint32_t model, float fX, float fY, float fZ, float fRadius)
{
	Log("Remove 1");
	for(size_t i = 0; i < AddedEntities.size(); i++)
	{
		Log("Remove 2 | Entity: 0x%X", (uintptr_t)AddedEntities.at(i));
		float fDistanceX = AddedEntities.at(i)->mat->pos.X;
		float fDistanceY = AddedEntities.at(i)->mat->pos.Y;
		float fDistanceZ = AddedEntities.at(i)->mat->pos.Z;
		Log("Remove 3");
		...
		{
			Log("Remove 5");
			if(AddedEntities.at(i)->nModelIndex == model)
			{
				Log("Remove 6");
				Log("I found object to remove!");
			}
		}
